...
std::shared_ptr<const UIState> UIStateManager::state(uint32_t id) const {
...
void UIStateManager::setState(uint32_t id, StateFlags flags) {
...
void UIStateManager::addState(uint32_t id, StateFlags flags) {
    auto &ptr = m_stateMap[id];
    if (!ptr)
...
void UIStateManager::removeState(uint32_t id, StateFlags flags) {
    auto &ptr = m_stateMap[id];
    if (!ptr)
        return;
    ptr->flags &= ~flags;
