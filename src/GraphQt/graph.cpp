...
Graph::Graph(QObject *parent)
    ...
    connect(m_layout.data(), &GraphLayout::validChanged, this, &Graph::updateModels);
}

QUrl Graph::source() const { return m_source; }
...
GraphLayout *Graph::layout() const { return m_layout.data(); }

NodeListModel *Graph::nodes() { return &m_nodeModel; }

EdgeListModel *Graph::edges() { return &m_edgeModel; }

ogdf::Graph &Graph::g() { return m_graph; }

ogdf::node Graph::v(const QString &node) const { return m_vs.value(node); }

QString Graph::node(ogdf::node v) const { return m_nodes[v]; }
...
ogdf::edge Graph::e(const QString &edge) const { return m_es.value(edge); }

QString Graph::edge(ogdf::edge e) const { return m_edges[e]; }
...
bool Graph::save(const QUrl &url) {
    ...
    QByteArray ba = path.toLocal8Bit(); // safe conversion
    std::ofstream ofs(ba.constData());
    bool result = ogdf::GraphIO::writeGML(m_attributes, ofs);
...
bool Graph::reload() {
    ...
    QString path = m_source.toLocalFile();
    QByteArray ba = path.toLocal8Bit(); // safe conversion
    std::ifstream ifs(ba.constData());
    return ogdf::GraphIO::readGML(m_attributes, m_graph, ifs);
}
...
void Graph::randomSimpleGraph(int n, int m) {
    ...
    if (ogdf::randomSimpleGraph(m_graph, n, m)) {
        ...
        qmlInfo(this) << "Can not generate a simple graph with " << n << " nodes and "
                      << m << " edges";
    }
}
...
void Graph::randomHierarchy(int n,
                            int m,
                            bool planar,
                            bool singleSource,
                            bool longEdges) {
    clear();
    ogdf::randomHierarchy(m_graph, n, m, planar, singleSource, longEdges);
    m_layout->invalidate();
}
...
void Graph::clear() { m_graph.clear(); }
